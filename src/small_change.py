...
noun_exo_pat = re.compile(r'(ga|o|ni)="exo." ')
...
def convert_passive_file(in_path,active_out_path,passive_out_path,knp_dict,knp_tag_dict):
    ...
    # joined_sentence = re.sub(noun_exo_pat, '', joined_sentence)
    # active_joined_sentence = re.sub(noun_exo_pat, '', joined_sentence)
...
def main():
    ...
    for path in ntc_pathlist:
        # if(os.path.split(path)[1] not in knp_dict):
        #     continue
        active_out_path = 'edited_corpus/mapped_excl_exo/active/train/' + os.path.split(path)[1]
        passive_out_path = 'edited_corpus/mapped_excl_exo/passive/train/' + os.path.split(path)[1]
